...
export default class PermissionSetCmp extends LightningElement {
    ...
    @track readOnlyUser = [];
    columns = COLS;
    componentTitle = "Permission Set Assignment";
            ...
            then((result) => {
                if (result.length > ZERO) {
                    for (let index = ZERO; index < result.length; index += ONE) {
                        if (result[index].permissionSetName === "MYOB_Integration_Read_Only") {
                            this.readOnlyUser.push(result[index]);
                        } else if (result[index].permissionSetName === "MYOB_Integration_Operational") {
                            this.operationalUser.push(result[index]);
                        } else if (result[index].permissionSetName === "MYOB_Integration_Admin") {
